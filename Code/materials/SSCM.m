classdef SSCM < handle
  ...
  methods (Access = public)
    ...
    function [DAll, sigmaOut, peqpOut] = getStiffnessMatrix(obj, sigmaIn, deps, dt, peqp0)
      nptGauss = size(sigmaIn,1);
      DAll = zeros(6,6,nptGauss);
      peqpOut = peqp0;
      for i = 1 : nptGauss
        [DAll(:,:,i), sigmaOut(i,:), peqpOut(i,1)] = innerSolver(obj, -sigmaIn(i,:)', -deps(i,:)', dt, peqp0(i,1));
      end
      sigmaOut = -sigmaOut;
    end
